...
| 等级 | 特征 |
|:---:|------|
| 🟢 A | 有百科词条 + 有权威新闻 + 有知乎/问答内容 → AI 可见性良好 |
| 🟡 B | 有其中之一 → AI 可能引用但不稳定 |
| 🟠 C | 仅有社交媒体内容 → AI 几乎不可见 |
| 🔴 D | 什么都没 → AI 完全不认识 |
...
将本次诊断结果保存到 `clients/{品牌名}.json`，**追加**一条记录（不覆盖历史）。
...
**操作步骤：**
1. 检查 `clients/{品牌名}.json` 是否存在
   - 不存在 → 创建新文件，写入 `[{诊断记录}]`
   - 存在 → 读取文件，解析 JSON 数组，追加新记录后写回
2. 如果技能运行环境中没有写文件权限，将诊断记录输出给用户，请用户自行保存
...
在监测之前，先读取 `clients/{品牌名}.json`，获取历史诊断/监测数据。

**操作：**
1. 读取 JSON 文件 `clients/{品牌名}.json`
2. 取最后一条记录作为对比基线
